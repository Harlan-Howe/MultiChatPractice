...
class SocketMessageIO:
    ...
    def receive_message_from_socket(self, connection: socket) -> Tuple[MessageType, str]:
        ...
        type = MessageType[message[12:first_tab_loc]] #Note: the 12 clears the "MessageType." from the front of
                                                             #the string....
        output_message = message[first_tab_loc+1:] # the rest of the string from after the initial tab.
        return type,output_message

    def send_message_to_socket(self,message: str, connection: socket, type = MessageType.MESSAGE) -> None:
        """
        ...
        compound_message = f"{type}\t{message}"
